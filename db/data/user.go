...
func GetUser(user *User) error {
	...
	return db.Bun.NewRaw(query, user.ID).Scan(context.Background(), user)
}

func GetUserById[T string | uuid.UUID](id T) (User, error) {
	var user User
	query := "SELECT * FROM users WHERE id = ?"
	err := db.Bun.NewRaw(query, id).Scan(context.Background(), &user)
	return user, err
}
...
func GetUserByEmail(email string) (User, error) {
	...
	err := db.Bun.NewRaw(query, email).Scan(context.Background(), &user)
	return user, err
}
...
func IsUserExists(email string) (bool, error) {
	...
	err := db.Bun.NewRaw(query, email).Scan(context.Background(), &exists)
	if err != nil {
		return false, err
	}
	return exists, nil
